...
function cleanRegisterForm() { registerForm.reset(); document.querySelectorAll(".register-box .input-error").forEach((error) => error.classList.remove("show")); }
...
registerForm.addEventListener("submit", (event) => {
  ...
  const newUser = { name: registerName.value.trim(), lastName: registerLastName.value.trim(), email: registerEmail.value.trim(), password: registerPassword.value, accounts: [{ type: "Checking", number: generateUniqueAccountNumber(users), balance: 1000 }, { type: "Savings", number: generateUniqueAccountNumber(users), balance: 0 }], transactions: [] };
...
transferForm.addEventListener("submit", (event) => {
  ...
  const senderUser = users.find((user) => user.email === currentUser?.email); const senderAccount = senderUser?.accounts.find((account) => account.number === sourceAccountNumber);
  if (!senderAccount) { transferAmountError.textContent = "Please choose an account"; transferAmountError.classList.add("show"); return; }
  const recipientUser = users.find((user) => { const hasAccount = user.accounts?.some((account) => account.number === recipientAccountNumber); return user.name.toLowerCase() === transferName.value.trim().toLowerCase() && user.lastName.toLowerCase() === transferLastName.value.trim().toLowerCase() && hasAccount; });
  if (!recipientUser) { recipientTransferError.textContent = "No matching recipient found"; recipientTransferError.classList.add("show"); return; }
  const recipientAccount = recipientUser.accounts.find((account) => account.number === recipientAccountNumber);
  if (senderUser.email === recipientUser.email && senderAccount.number === recipientAccount.number) { recipientTransferError.textContent = "You cannot transfer money to the same account"; recipientTransferError.classList.add("show"); return; }
  if (Number.isNaN(amount) || amount <= 0) { transferAmountError.textContent = "The amount has to be more than 0"; transferAmountError.classList.add("show"); return; }
  if (amount > senderAccount.balance) { transferAmountError.textContent = "Not enough funds"; transferAmountError.classList.add("show"); return; }
  senderAccount.balance -= amount; recipientAccount.balance += amount;
  const timestamp = new Date().toISOString(); senderUser.transactions ??= []; recipientUser.transactions ??= [];
  ...
  localStorage.setItem("users", JSON.stringify(users)); localStorage.setItem("currentUser", JSON.stringify(senderUser));
  transferMessage.textContent = "Transaction successful!"; transferMessage.classList.add("show"); transferForm.reset(); setTimeout(() => transferMessage.classList.remove("show"), 3000);
});
